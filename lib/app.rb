...
#permet d'envoyer 5 tweets à 5 personnes de manière aléatoire
...
def twittag(client)
  ...
  client.update ("Salut les journalistes #bonjourmonde #{list.sample}")                                                                                                     

end

# 5.times {|i| twittag(login_twitter)}

# Permet de liker les 25 derniers tweets avec bonjour monde

def like_tweet(client)
  var = client.search("#bonjour_monde", result_type: "recent").take(25)
  client.favorite(var)
end

#like_tweet(login_twitter) est l'éxécution

def follow_tweet (client)
  var = client.search("#bonjour_monde", result_type: "recent").take(20).collect do |tweet|
